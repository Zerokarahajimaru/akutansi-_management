...
<div class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(169,141,102,0.08)] overflow-hidden">
 ...
 <div>
 <h2 class="text-[#A98D66] font-black tracking-tightest uppercase text-lg">Data Transaksi Pembelian</h2>
 <p class="text-slate-500 text-xs font-medium">Kelola riwayat pengadaan stok barang dari pemasok Xyra.id</p>
 </div>
 </div>
            ...
            <div x-data="{ openImport: false, isDragging: false, isSubmitting: false }" class="flex-1 lg:flex-none">
                ...
                <div x-show="openImport" x-cloak class="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4" x-transition.opacity>
                    <div @click.outside="openImport = false" class="bg-white rounded-3xl border border-slate-100 shadow-xl w-full max-w-md p-6 transform transition-all" x-transition.scale.95>
                        ...
                        <form action="{{ route('util.import', 'pembelian') }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true" class="space-y-4">
                            @csrf
                            <div 
                            ...
                            >
                                <div 
                                    ...
                                    :class="isDragging ? 'bg-white/50 border-[#3B8A7F]' : 'border-slate-300 group-hover:border-[#3B8A7F]'"
                                >
                                    <input 
                                        type="file" 
                                        name="csv_file" 
                                    ...
                                    <div class="relative z-0">
                                        ...
                                        <p id="fileNamePembelian" class="text-xs font-bold text-slate-800 mt-3 truncate"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
                                <button type="button" @click="openImport = false" class="w-full sm:w-auto bg-slate-100 text-slate-800 border border-slate-200 font-bold rounded-xl px-5 py-2.5 text-sm hover:bg-slate-200 transition-all">Batal</button>
 ...
 <div class="overflow-x-auto custom-scrollbar relative">
    <table id="pembelian-table" class="w-full text-left border-separate border-spacing-0">
    <thead>
    @php
    ...
    <th class="py-6 px-8 cursor-pointer group sticky left-0 bg-[#B04025] z-20 shadow-[4px_0_10px_-3px_rgba(0,0,0,0.2)] whitespace-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Tgl_Pembelian', 'sort_dir' => $newDir, 'page' => 1]) }}'">
            <div class="flex items-center">
                Tanggal
                <i class="fas {{ $currentSortBy === 'Tgl_Pembelian' ? ($currentSortDir === 'asc' ? 'fa-sort-up text-white' : 'fa-sort-down text-white') : 'fa-sort text-white/30' }} text-[10px] ml-auto group-hover:text-white transition-colors"></i>
            </div>
        </th>
        <th class="py-6 px-8 whitespace-nowrap">Informasi Produk</th>
        <th class="py-6 px-8 whitespace-nowrap">Data Pemasok</th>
        <th class="py-6 px-8 whitespace-nowrap">Metode Pembayaran</th>
        <th class="py-6 px-8 text-center whitespace-nowrap">Kuantitas</th>
        <th class="py-6 px-8 whitespace-nowrap">Admin Pencatat</th>
        <th class="py-6 px-8 text-right whitespace-nowrap">Total Harga</th>
 ...
 <td class="py-5 px-8 text-sm text-slate-800 font-medium sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-[#F6F4F0] transition-colors whitespace-nowrap">{{ $item->Tgl_Pembelian ? date('d/m/Y', strtotime($item->Tgl_Pembelian)) : '-' }}</td>
 ...
 <td class="py-5 px-8 whitespace-nowrap">
    <div class="flex flex-col min-w-[150px]">
        <p class="text-sm text-slate-800 font-semibold max-w-xs truncate" title="{{ $item->pemasok->Nama_Pemasok ?? 'Pemasok Dihapus' }}">{{ $item->pemasok->Nama_Pemasok ?? 'Pemasok Dihapus' }}</p>
        <p class="text-[10px] text-slate-800 font-bold uppercase tracking-tighter">{{ $item->ID_Pemasok ?? '-' }}</p>
    </div>
 </td>
 <td class="py-5 px-8 whitespace-nowrap">
 ...
 <span class="text-sm font-bold text-slate-800">+{{ $item->Kuantitas ?? 0 }}</span>
 </td>
 <td class="py-5 px-8 whitespace-nowrap">
    <div class="flex flex-col min-w-[120px]">
        <span class="text-xs font-medium text-slate-800">{{ $item->user->name ?? 'Sistem' }}</span>
        <span class="text-[10px] text-slate-800 font-mono font-bold">{{ $item->user_id ?? '-' }}</span>
    </div>
 </td>
 <td class="py-5 px-8 text-sm font-bold text-slate-800 text-right whitespace-nowrap">Rp {{ number_format($item->Total_Harga ?? 0, 0, ',', '.') }}</td>
 <td class="py-5 px-8 text-center whitespace-nowrap">
 <div class="flex justify-center space-x-2">
 <a href="{{ route('data.pembelian.edit', $item->ID_Pembelian) }}" wire:navigate class="flex items-center justify-center w-8 h-8 rounded-xl bg-slate-50 text-slate-800 hover:bg-[#3B8A7F] hover:text-white shadow-sm transition-all" title="Ubah Data">
