...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100">
 ...
 <div>
 <h2 class="font-bold text-gray-800 text-lg">Data Transaksi Pembelian</h2>
 <p class="text-gray-500 text-xs">Kelola riwayat pengadaan stok barang dari pemasok Xyra.id</p>
 </div>
 ...
 <a href="{{ route('util.template', 'pembelian') }}" class="flex items-center px-4 py-2 bg-gray-50 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-100 transition-colors border border-gray-200">
     ...
     <div x-show="openImport" x-cloak class="fixed inset-0 z-[150] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm" x-transition.opacity>
         <div @click.outside="openImport = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all" x-transition.scale.95>
             <div class="flex justify-between items-center mb-5">
                 <h3 class="text-lg font-bold text-gray-800">Impor Data Pembelian</h3>
             ...
             <form action="{{ route('util.import', 'pembelian') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                 ...
                 <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-teal-500 hover:bg-teal-50/50 transition-colors cursor-pointer relative">
                     <input type="file" name="csv_file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".xlsx,.xls,.csv" required onchange="document.getElementById('fileNamePembelian').textContent = this.files[0].name">
 ...
 <td class="py-4 px-6 text-sm text-gray-600 font-medium">{{ date('d/m/Y', strtotime($item->Tgl_Pembelian)) }}</td>
 <td class="py-4 px-6">
 ...
 <td class="py-4 px-6">
    <div class="flex flex-col">
        <p class="text-sm text-gray-700 font-semibold">{{ $item->pemasok->Nama_Pemasok ?? 'Pemasok Dihapus' }}</p>
 ...
 <td class="py-4 px-6">
    <div class="items-center">
        <span class="text-xs font-medium text-gray-600">{{ $item->user->name ?? 'Sistem' }}</span>
        <span class="ml-1.5 text-[9px] text-gray-400 font-mono">({{ $item->user->formatted_id ?? '-' }})</span>
    </div>
 </td>
 <td class="py-4 px-6 text-sm font-bold text-gray-900">Rp {{ number_format($item->Total_Harga, 0, ',', '.') }}</td>
 <td class="py-4 px-6 text-center">
 <div class="flex justify-center space-x-2">
 <a href="{{ route('data.pembelian.edit', $item->ID_Pembelian) }}" wire:navigate.hover class="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors" title="Ubah Data">
 ...
 <tr class="empty-state">
    <td colspan="100%" class="py-16 text-center">
        <div class="flex flex-col items-center justify-center">
            ...
            <h3 class="text-gray-800 font-bold text-base">Riwayat Pembelian Kosong</h3>
            <p class="text-gray-400 text-sm mt-1">Belum ada transaksi pembelian untuk ditampilkan pada periode ini.</p>
        </div>
    </td>
 </tr>
 @endforelse
