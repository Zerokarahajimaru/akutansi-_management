...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="font-bold text-slate-800 text-lg">Riwayat Pembelian Barang</h2>
            <p class="text-slate-500 text-xs">Daftar semua transaksi stok masuk (Purchase)</p>
        </div>
        <div class="flex flex-wrap gap-2" x-data="{ 
            ...
            <a href="{{ route('util.export', 'pembelian') }}" class="flex items-center px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold hover:bg-emerald-100 transition-colors">
                <i class="fas fa-file-export mr-2"></i> Export
            </a>
            <a href="{{ route('util.template', 'pembelian') }}" class="flex items-center px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-colors">
            ...
            <a href="{{ route('input.pembelian') }}" class="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="py-4 px-6 text-sm text-slate-500">{{ \Carbon\Carbon::parse($item->Tgl_Pembelian)->format('d/m/Y') }}</td>
                    <td class="py-4 px-6">
                        <p class="text-sm font-bold text-slate-800">{{ $item->dataBarang->Nama_Barang ?? '-' }}</p>
                        <p class="text-[10px] text-slate-400 font-medium">{{ $item->ID_Pembelian }}</p>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6 text-sm text-slate-600">{{ $item->pemasok->Nama_Pemasok ?? '-' }}</td>
                    <td class="py-4 px-6 text-center">
                        <span class="text-emerald-500 font-black text-sm">+{{ $item->Kuantitas }}</span>
                    </td>
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex justify-center space-x-2">
                            <a href="{{ route('data.pembelian.edit', $item->ID_Pembelian) }}" class="flex items-center justify-center w-8 h-8 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white transition-colors">
                                <i class="fas fa-edit text-xs"></i>
                            </a>
                            <form action="{{ route('data.pembelian.destroy', $item->ID_Pembelian) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus transaksi ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="{{ Auth::user()->role === 'admin' ? 9 : 8 }}" class="py-10 text-center text-slate-400 italic text-sm">Belum ada data pembelian</td>
                </tr>
                @endforelse
            </tbody>
        </table>
