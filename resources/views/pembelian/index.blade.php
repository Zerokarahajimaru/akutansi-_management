...
<div class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(13,148,136,0.05)]">
 ...
 <th class="py-4 px-6 cursor-pointer group sticky left-0 bg-teal-50 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Tgl_Pembelian', 'sort_dir' => $newDir, 'page' => 1]) }}'">
 ...
 <td class="py-4 px-6 text-sm text-slate-600 font-medium sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-teal-50 whitespace-nowrap">{{ $item->Tgl_Pembelian ? date('d/m/Y', strtotime($item->Tgl_Pembelian)) : '-' }}</td>
 ...
 <td class="py-4 px-6 whitespace-nowrap">
    <div class="flex flex-col min-w-[150px]">
        <p class="text-sm text-slate-700 font-semibold max-w-xs truncate" title="{{ $item->pemasok->Nama_Pemasok ?? 'Pemasok Dihapus' }}">{{ $item->pemasok->Nama_Pemasok ?? 'Pemasok Dihapus' }}</p>
 ...
 <span class="text-sm font-bold text-teal-600">+{{ $item->Kuantitas ?? 0 }}</span>
 </td>
 <td class="py-4 px-6 whitespace-nowrap">
 ...
 <td class="py-4 px-6 text-sm font-bold text-slate-900 text-right whitespace-nowrap">Rp {{ number_format($item->Total_Harga ?? 0, 0, ',', '.') }}</td>
 <td class="py-4 px-6 text-center whitespace-nowrap">
 <div class="flex justify-center space-x-2">
 <a href="{{ route('data.pembelian.edit', $item->ID_Pembelian) }}" wire:navigate class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 text-teal-600 hover:bg-teal-600 hover:text-white shadow-sm transition-all" title="Ubah Data">
