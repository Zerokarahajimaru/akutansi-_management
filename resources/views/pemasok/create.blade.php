...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(169,141,102,0.08)] overflow-hidden">
        <div class="p-10 border-b border-slate-50 bg-slate-50/20">
            <h2 class="text-2xl font-black text-[#A98D66] tracking-tightest uppercase">Pendaftaran <span class="text-[#3B8A7F]">Pemasok Baru.</span></h2>
            <p class="text-slate-500 text-xs font-medium mt-1 leading-relaxed">Tambahkan informasi mitra pemasok barang untuk inventaris Xyra.id</p>
        </div>
        <form action="{{ route('input.pemasok') }}" method="POST" class="p-10 space-y-6" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="NoTelp_Pemasok" value="{{ old('NoTelp_Pemasok') }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-2xl px-5 py-4 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-[#ca5b33]/15 focus:border-[#ca5b33] transition-all duration-300 @error('NoTelp_Pemasok') border-[#B04025]/40 focus:ring-[#B04025]/10 focus:border-[#B04025] @enderror" placeholder="08xxxxxxxxxx" required>
