...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="font-bold text-slate-800 text-lg">Daftar Pemasok</h2>
            <p class="text-slate-500 text-xs">Kelola data mitra penyuplai barang</p>
        </div>
        <div class="flex flex-wrap gap-2" x-data="{ 
            ...
            <a href="{{ route('util.export', 'pemasok') }}" class="flex items-center px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold hover:bg-emerald-100 transition-colors">
                <i class="fas fa-file-export mr-2"></i> Export
            </a>
            <a href="{{ route('util.template', 'pemasok') }}" class="flex items-center px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-colors">
            ...
            <a href="{{ route('input.pemasok') }}" class="flex items-center px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="py-4 px-6 text-center">
                        <div class="flex justify-center space-x-2">
                            <a href="{{ route('data.pemasok.edit', $p->ID_Pemasok) }}" class="flex items-center justify-center w-8 h-8 rounded-lg bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white transition-colors">
                                <i class="fas fa-edit text-xs"></i>
                            </a>
                            <form action="{{ route('data.pemasok.destroy', $p->ID_Pemasok) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pemasok ini?')">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="4" class="py-10 text-center text-slate-400 italic text-sm">Belum ada data pemasok</td>
                </tr>
                @endforelse
            </tbody>
        </table>
