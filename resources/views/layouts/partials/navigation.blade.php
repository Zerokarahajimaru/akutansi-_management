<nav class="mt-4 space-y-1 pb-10 custom-scrollbar overflow-x-hidden">
 ...
 <a href="{{ route('dashboard') }}" wire:navigate @click="sidebarOpen = false" 
    ...
    <div class="flex items-center" :class="sidebarOpen ? '' : 'justify-center w-full'">
        <i class="fas fa-chart-line w-5 text-center transition-transform group-hover:scale-110 {{ Request::is('/') ? 'text-teal-600' : 'text-teal-600 group-hover:text-teal-500' }}"></i>
        <span x-show="sidebarOpen" x-transition.opacity.duration.300 class="text-sm font-bold ml-4 whitespace-nowrap tracking-tight">Dashboard Analitik</span>
    </div>
 </a>
 ...
 <button @click="if(!sidebarOpen) { sidebarOpen = true; open = true; } else { open = !open; }" 
    ...
    class="flex items-center justify-between w-full py-3.5 px-6 transition-all duration-300 group {{ Request::is('data/*') ? 'bg-teal-100/80 text-teal-900 border-r-4 border-teal-600' : 'text-slate-600 hover:text-teal-700 hover:bg-teal-100/50' }}"
    ...
    <div class="flex items-center" :class="sidebarOpen ? '' : 'justify-center w-full'">
        ...
        <span x-show="sidebarOpen" x-transition.opacity.duration.300 class="text-sm font-bold ml-4 whitespace-nowrap tracking-tight">Daftar Master Data</span>
    </div>
