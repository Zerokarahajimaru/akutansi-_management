<nav class="mt-4 space-y-1.5 pb-10">
 ...
 <button @click="sidebarOpen ? (open = !open) : (sidebarOpen = true, open = true)" 
    class="flex items-center justify-between w-full py-3.5 px-4 transition-all duration-300 group {{ Request::is('data/*') ? 'bg-teal-100/80 text-teal-900 border-r-4 border-teal-600 rounded-l-xl rounded-r-none' : 'text-slate-600 hover:text-teal-700 hover:bg-teal-100/50 rounded-xl' }}">
 ...
 <template x-if="open && sidebarOpen">
    ...
    ['route' => 'data.pembelian', 'label' => 'Pembelian', 'icon' => 'fa-cart-shopping', 'pattern' => 'data/pembelian*'],
    ['route' => 'data.penjualan', 'label' => 'Penjualan', 'icon' => 'fa-file-invoice-dollar', 'pattern' => 'data/penjualan*'],
    ];
    @endphp
    @foreach($dataLinks as $link)
    <a href="{{ route($link['route']) }}" wire:navigate @click="if(window.innerWidth < 1024) sidebarOpen = false"
        class="flex items-center py-2.5 px-3 text-xs font-bold rounded-lg transition-all {{ Request::is($link['pattern']) ? 'bg-teal-600 text-white shadow-md shadow-teal-500/20 rounded-xl' : 'text-slate-500 hover:text-teal-600 hover:bg-teal-50' }}">
 ...
 <button @click="sidebarOpen ? (open = !open) : (sidebarOpen = true, open = true)" 
    class="flex items-center justify-between w-full py-3.5 px-4 transition-all duration-300 group {{ Request::is('input/*') ? 'bg-teal-100/80 text-teal-900 border-r-4 border-teal-600 rounded-l-xl rounded-r-none' : 'text-slate-600 hover:text-teal-700 hover:bg-teal-100/50 rounded-xl' }}">
    <div class="flex items-center">
 ...
 <button @click="sidebarOpen ? (open = !open) : (sidebarOpen = true, open = true)" 
    class="flex items-center justify-between w-full py-3.5 px-4 transition-all duration-300 group {{ Request::is('laporan/*') ? 'bg-teal-100/80 text-teal-900 border-r-4 border-teal-600 rounded-l-xl rounded-r-none' : 'text-slate-600 hover:text-teal-700 hover:bg-teal-100/50 rounded-xl' }}">
    <div class="flex items-center">
 ...
 <template x-if="open && sidebarOpen">
    ...
    <a href="{{ route('laporan.pembelian') }}" wire:navigate @click="if(window.innerWidth < 1024) sidebarOpen = false"
        class="flex items-center py-2.5 px-3 text-xs font-bold rounded-lg transition-all {{ Request::is('laporan/pembelian') ? 'bg-teal-600 text-white shadow-md shadow-teal-500/20 rounded-xl' : 'text-slate-500 hover:text-teal-600 hover:bg-teal-50' }}">
    ...
    <a href="{{ route('laporan.penjualan') }}" wire:navigate @click="if(window.innerWidth < 1024) sidebarOpen = false"
        class="flex items-center py-2.5 px-3 text-xs font-bold rounded-lg transition-all {{ Request::is('laporan/penjualan') ? 'bg-teal-600 text-white shadow-md shadow-teal-500/20 rounded-xl' : 'text-slate-500 hover:text-teal-600 hover:bg-teal-50' }}">
    ...
    <a href="{{ route('laporan.stok') }}" wire:navigate @click="if(window.innerWidth < 1024) sidebarOpen = false"
        class="flex items-center py-2.5 px-3 text-xs font-bold rounded-lg transition-all {{ Request::is('laporan/stok') ? 'bg-teal-600 text-white shadow-md shadow-teal-500/20 rounded-xl' : 'text-slate-500 hover:text-teal-600 hover:bg-teal-50' }}">
