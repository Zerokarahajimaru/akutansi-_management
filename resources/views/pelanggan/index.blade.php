...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100">
 ...
 <div>
 <h2 class="font-bold text-gray-800 text-lg">Kelola Data Pelanggan</h2>
 <p class="text-gray-500 text-xs">Daftar pelanggan aktif untuk transaksi penjualan Xyra.id</p>
 </div>
 ...
 <a href="{{ route('util.template', 'pelanggan') }}" class="flex items-center px-4 py-2 bg-gray-50 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-100 transition-colors border border-gray-200">
     ...
     <div x-show="openImport" x-cloak class="fixed inset-0 z-[150] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm" x-transition.opacity>
         <div @click.outside="openImport = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 transform transition-all" x-transition.scale.95>
             <div class="flex justify-between items-center mb-5">
                 <h3 class="text-lg font-bold text-gray-800">Impor Data Pelanggan</h3>
             ...
             <form action="{{ route('util.import', 'pelanggan') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                 ...
                 <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-teal-500 hover:bg-teal-50/50 transition-colors cursor-pointer relative">
                     <input type="file" name="csv_file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".xlsx,.xls,.csv" required onchange="document.getElementById('fileNamePelanggan').textContent = this.files[0].name">
 ...
 <a href="{{ route('input.pelanggan') }}" wire:navigate.hover class="flex items-center px-4 py-2 bg-teal-600 text-white rounded-xl text-xs font-bold hover:bg-teal-700 transition-colors shadow-lg shadow-teal-200">
 ...
 <th class="py-4 px-6 cursor-pointer group" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Nama_Pelanggan', 'sort_dir' => $newDir, 'page' => 1]) }}'">
    <div class="flex items-center">
        Nama Lengkap
        <i class="fas {{ $currentSortBy === 'Nama_Pelanggan' ? ($currentSortDir === 'asc' ? 'fa-sort-up text-teal-600' : 'fa-sort-down text-teal-600') : 'fa-sort text-gray-300' }} text-[10px] ml-auto group-hover:text-teal-500 transition-colors"></i>
    </div>
 </th>
 ...
 <td class="py-4 px-6 text-sm text-teal-600 font-bold uppercase">{{ $p->ID_Pelanggan }}</td>
 <td class="py-4 px-6 text-sm font-bold text-gray-800">{{ $p->Nama_Pelanggan }}</td>
 <td class="py-4 px-6 text-sm text-gray-600 leading-relaxed">{{ $p->Alamat_Pelanggan }}</td>
 <td class="py-4 px-6 text-sm text-gray-600 text-center italic">{{ $p->NoTelp_Pelanggan }}</td>
 <td class="py-4 px-6 text-center">
 <div class="flex justify-center space-x-2">
 <a href="{{ route('data.pelanggan.edit', $p->ID_Pelanggan) }}" wire:navigate.hover class="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors" title="Ubah Data">
 ...
 <i class="fas fa-address-book text-gray-300 text-3xl"></i>
 </div>
 <h3 class="text-gray-800 font-bold text-base">Data Pelanggan Kosong</h3>
 <p class="text-gray-400 text-sm mt-1">Gunakan tombol di atas untuk mendaftarkan pelanggan baru.</p>
 </div>
 </td>
 </tr>
 @endforelse
