...
<div class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(13,148,136,0.05)]">
 ...
 <div class="flex flex-col xl:flex-row justify-between items-center gap-4 mb-6 w-full">
    <form method="GET" action="" class="w-full sm:w-[320px] lg:w-[400px] flex-shrink-0 relative">
        ...
        <input type="text" name="search" value="{{ request('search') }}" 
            class="w-full bg-white border border-slate-200 text-slate-800 text-sm rounded-xl pl-10 pr-4 py-2.5 focus:outline-none focus:ring-4 focus:ring-[#ca5b33]/10 focus:border-[#ca5b33] transition-all shadow-sm placeholder:text-slate-400" 
    ...
    <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <div @click.outside="openImport = false" class="bg-white rounded-2xl border border-slate-100 shadow-xl w-full max-w-md p-6 transform transition-all" x-transition.scale.95>
                        ...
                        <form action="{{ route('util.import', 'pelanggan') }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true" class="space-y-4">
                            ...
                            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
                                ...
                                <button type="submit" :disabled="isSubmitting" :class="isSubmitting ? 'opacity-70 cursor-not-allowed' : ''" class="w-full sm:w-auto bg-[#ca5b33] text-white font-black rounded-xl shadow-lg shadow-[#ca5b33]/20 hover:bg-[#B04025] hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-300 px-5 py-2.5 text-sm flex items-center justify-center group uppercase tracking-widest">
        ...
        <a href="{{ route('input.pelanggan') }}" wire:navigate.hover class="w-full sm:w-auto bg-[#ca5b33] text-white font-black rounded-xl shadow-lg shadow-[#ca5b33]/25 hover:bg-[#B04025] hover:-translate-y-0.5 active:scale-95 transition-all duration-300 px-6 py-3 text-[10px] uppercase tracking-widest flex items-center justify-center">
 ...
 @endphp
 <tr class="bg-white border-b-2 border-[#B04025] text-[#000000] text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
 <th class="py-4 px-6 sticky left-0 bg-white z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap">ID Pelanggan</th>
 ...
 <a href="{{ route('data.pelanggan.edit', $p->ID_Pelanggan) }}" wire:navigate class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-[#ca5b33] hover:text-white shadow-sm transition-all" title="Ubah Data">
 <i class="fas fa-edit text-xs"></i>
 </a>
 <form action="{{ route('data.pelanggan.destroy', $p->ID_Pelanggan) }}" method="POST">
 @csrf
 @method('DELETE')
 <button type="submit" class="w-8 h-8 rounded-lg bg-slate-50 text-[#B04025] hover:bg-[#B04025] hover:text-white shadow-sm transition-all" title="Hapus Data">
