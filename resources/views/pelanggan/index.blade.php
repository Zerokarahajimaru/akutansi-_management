...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100">
 ...
 @php $newDir = ($currentSortBy === 'Alamat_Pelanggan' && $currentSortDir === 'asc') ? 'desc' : 'asc'; @endphp
 <th class="py-4 px-6 cursor-pointer group" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Alamat_Pelanggan', 'sort_dir' => $newDir, 'page' => 1]) }}'">
    <div class="flex items-center">
        Alamat
        <i class="fas {{ $currentSortBy === 'Alamat_Pelanggan' ? ($currentSortDir === 'asc' ? 'fa-sort-up text-teal-600' : 'fa-sort-down text-teal-600') : 'fa-sort text-gray-300' }} text-[10px] ml-auto group-hover:text-teal-500 transition-colors"></i>
    </div>
 </th>
 @php $newDir = ($currentSortBy === 'NoTelp_Pelanggan' && $currentSortDir === 'asc') ? 'desc' : 'asc'; @endphp
 <th class="py-4 px-6 cursor-pointer group" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'NoTelp_Pelanggan', 'sort_dir' => $newDir, 'page' => 1]) }}'">
    <div class="flex items-center">
        No. HP
        <i class="fas {{ $currentSortBy === 'NoTelp_Pelanggan' ? ($currentSortDir === 'asc' ? 'fa-sort-up text-teal-600' : 'fa-sort-down text-teal-600') : 'fa-sort text-gray-300' }} text-[10px] ml-auto group-hover:text-teal-500 transition-colors"></i>
    </div>
 </th>
 ...
 <a href="{{ route('data.pelanggan.edit', $p->ID_Pelanggan) }}" wire:navigate.hover class="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-colors">
 ...
 <a href="{{ route('input.pelanggan') }}" wire:navigate.hover class="mt-4 text-teal-600 text-xs font-bold hover:underline">Tambah Pelanggan Sekarang</a>
 </div>
 </td>
 </tr>
 @endforelse
