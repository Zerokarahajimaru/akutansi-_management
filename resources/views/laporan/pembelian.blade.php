    ...
    <div class="bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgba(13,148,136,0.05)] border border-slate-100">
        ...
        <div class="flex flex-col sm:flex-row justify-start xl:justify-end w-full">
            <div class="inline-flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm w-full sm:w-auto">
                <a href="{{ route('laporan.export.excel', array_merge(request()->query(), ['type' => 'pembelian'])) }}" class="flex-1 sm:flex-none px-5 py-2.5 bg-slate-50 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 text-[10px] font-black uppercase tracking-widest transition-all border-r border-slate-200 flex items-center justify-center gap-2">
                    <i class="fas fa-file-excel text-emerald-600 text-sm"></i> Ekspor Excel
                </a>
                <a href="{{ route('laporan.export.pdf', array_merge(request()->query(), ['type' => 'pembelian'])) }}" class="flex-1 sm:flex-none px-5 py-2.5 bg-slate-50 hover:bg-rose-50 text-slate-600 hover:text-rose-700 text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="even:bg-slate-50/50 hover:bg-teal-50/60 transition-colors group">
                        <td class="py-5 px-8 text-sm text-slate-600 font-medium whitespace-nowrap sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-teal-50">{{ $p->Tgl_Pembelian ? date('d/m/Y', strtotime($p->Tgl_Pembelian)) : '-' }}</td>
                        ...
                        <td class="py-5 px-8 text-center whitespace-nowrap">
                            <span class="px-3 py-1 bg-teal-50 text-teal-600 rounded-xl text-xs font-black">+{{ $p->Kuantitas ?? 0 }}</span>
                        </td>
                        <td class="py-5 px-8 whitespace-nowrap">
                            <div class="flex flex-col min-w-[150px]">
                                <span class="text-sm text-slate-700 font-semibold max-w-xs truncate" title="{{ $p->pemasok->Nama_Pemasok ?? '' }}">{{ $p->pemasok->Nama_Pemasok ?? '-' }}</span>
