...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="p-6 border-b border-slate-50">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h2 class="font-bold text-slate-800 text-lg">Laporan Penjualan Produk</h2>
                <p class="text-slate-500 text-xs">Analisa performa penjualan dan pendapatan toko</p>
            </div>
            <form action="{{ route('laporan.penjualan') }}" method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('util.export', 'penjualan') }}" class="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-xl text-xs font-black hover:bg-emerald-100 transition-all flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 border-b border-slate-50">
        <div class="p-6 border-r border-slate-50">
            ...
            <h3 class="text-xl font-black text-slate-800">{{ $penjualans->count() }}</h3>
        </div>
        <div class="p-6 border-r border-slate-50">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Produk Terjual</p>
            <h3 class="text-xl font-black text-rose-600">-{{ $penjualans->sum('Kuantitas') }} <span class="text-xs text-slate-400 font-medium">Unit</span></h3>
        </div>
        <div class="p-6">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Pendapatan (Gross)</p>
            <h3 class="text-xl font-black text-emerald-600">Rp {{ number_format($penjualans->sum('Total_Harga'), 0, ',', '.') }}</h3>
        </div>
    </div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    <td class="py-4 px-6 text-sm text-slate-500 font-medium">{{ \Carbon\Carbon::parse($item->Tanggal_Penjualan)->format('d/m/Y') }}</td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6 text-sm text-slate-600 font-medium">{{ $item->pelanggan->Nama_Pelanggan ?? '-' }}</td>
                    <td class="py-4 px-6 text-center">
                        <span class="px-2 py-1 bg-rose-50 text-rose-600 text-[11px] font-black rounded-lg">-{{ $item->Kuantitas }}</span>
                    </td>
                    <td class="py-4 px-6 text-sm font-black text-slate-900 text-right">Rp {{ number_format($item->Total_Harga, 0, ',', '.') }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="py-12 text-center text-slate-400 italic text-sm font-medium">Tidak ada data penjualan pada periode ini</td>
                </tr>
                @endforelse
            </tbody>
        </table>
