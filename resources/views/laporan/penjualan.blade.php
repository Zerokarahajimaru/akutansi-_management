...
<div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(15,23,42,0.04)] border border-slate-100 overflow-hidden">
 ...
 <button type="submit" class="bg-teal-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-teal-700 transition-all shadow-lg shadow-teal-500/30">
 FILTER
 </button>
 <a href="{{ route('util.export', 'penjualan') }}" class="bg-slate-100 text-slate-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all flex items-center">
 ...
 <h3 class="text-xl font-bold text-slate-800">{{ $penjualans->count() }}</h3>
 </div>
 <div class="p-6 border-r border-slate-50">
 <p class="text-xs font-bold text-slate-400 mb-1">Total Produk Terjual</p>
 <h3 class="text-xl font-bold text-red-600">-{{ $penjualans->sum('Kuantitas') }} <span class="text-xs text-slate-400 font-medium">Unit</span></h3>
 </div>
 <div class="p-6">
 <p class="text-xs font-bold text-slate-400 mb-1">Total Pendapatan (Gross)</p>
 ...
 <tr>
    <td colspan="100%" class="py-16 text-center">
        <div class="flex flex-col items-center justify-center">
            ...
            <p class="text-slate-400 text-sm mt-1">Belum ada catatan penjualan untuk ditampilkan pada periode ini.</p>
        </div>
    </td>
 </tr>
 @endforelse
