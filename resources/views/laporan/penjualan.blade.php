...
<div class="space-y-8">
        ...
        <div class="p-6 md:p-8 border-b border-slate-50 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Analisa Omzet Penjualan</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_50px_rgba(13,148,136,0.05)] overflow-hidden">
        <div class="p-6 md:p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center bg-slate-50/30 gap-4">
            <h3 class="font-bold text-slate-800 text-lg">Detail Transaksi Keluar</h3>
            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <a href="{{ route('util.export', 'penjualan') }}" class="flex items-center justify-center px-5 py-2.5 bg-slate-100 text-slate-600 border border-slate-200 font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95">
                    <i class="fas fa-file-excel mr-2"></i> Ekspor Excel
                </a>
                <button onclick="window.print()" class="flex items-center justify-center px-5 py-2.5 bg-white text-slate-600 border border-slate-200 font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-teal-50/80 text-slate-600 text-[10px] font-black uppercase tracking-widest whitespace-nowrap">
                        ...
                        <th class="py-5 px-8 text-right">Total Harga</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @forelse($penjualans as $p)
                    <tr class="hover:bg-teal-50/30 transition-colors group">
                        <td class="py-5 px-8 text-sm text-slate-600 font-medium whitespace-nowrap">{{ date('d/m/Y', strtotime($p->Tanggal_Penjualan)) }}</td>
                        <td class="py-5 px-8">
                        ...
                        <td class="py-5 px-8 text-center whitespace-nowrap">
                            <span class="px-3 py-1 bg-rose-50 text-rose-600 rounded-full text-xs font-black">-{{ $p->Kuantitas }}</span>
                        </td>
                        <td class="py-5 px-8">
                            <div class="flex flex-col min-w-[150px]">
