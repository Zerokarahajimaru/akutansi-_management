...
<body class="min-h-screen grid grid-cols-1 lg:grid-cols-2 font-sans text-slate-800 antialiased overflow-x-hidden">
    ...
    <section class="hidden lg:flex flex-col justify-between relative overflow-hidden bg-[#3B8A7F] p-16">
        ...
        <div class="relative z-10 flex items-center gap-6">
            ...
            <div>
                <h1 class="text-4xl font-black text-[#000000] tracking-tightest">Xyra<span class="text-[#3B8A7F]">.id</span></h1>
    ...
    <main class="w-full flex items-center justify-center bg-white p-8 sm:p-12 lg:p-16 relative">
        <div class="w-full max-w-[400px]">
            ...
            <div class="lg:hidden mb-12 flex flex-col items-center">
                ...
                <h2 class="text-3xl font-black text-[#000000] tracking-tightest">Xyra<span class="text-[#3B8A7F]">.id</span></h2>
                <p class="text-[10px] text-white/60 font-black uppercase tracking-[0.3em] mt-3">Inventory Portal</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        <span class="absolute inset-y-0 left-0 pl-5 flex items-center text-slate-300 group-focus-within:text-[#A98D66] transition-colors duration-300">
                            <i class="fas fa-user-shield"></i>
                        </span>
                        <input type="text" name="username" id="username" 
                            class="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-12 pr-5 py-4 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-[#3B8A7F]/10 focus:border-[#3B8A7F] transition-all duration-300 @error('username') border-white/40 focus:ring-white/10 focus:border-white @enderror" 
                    ...
                    @error('username')
                        <p class="text-[#B04025] text-[10px] font-bold mt-2 ml-1 flex items-center animate-shake">
                            <i class="fas fa-circle-exclamation mr-2 text-[#A98D66]"></i>{{ $message }}
                        </p>
                    @enderror
                </div>
                ...
                <div x-data="{ showPassword: false }" class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input :type="showPassword ? 'text' : 'password'" name="password" id="password" 
                            class="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-12 pr-14 py-4 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-[#3B8A7F]/10 focus:border-[#3B8A7F] transition-all duration-300 @error('password') border-white/40 focus:ring-white/10 focus:border-white @enderror" 
                            placeholder="••••••••" required>
                        <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-5 flex items-center text-slate-300 hover:text-[#A98D66] focus:outline-none transition-colors duration-300">
                ...
                <div class="flex items-center justify-between px-1">
                    <div class="flex items-center">
                        <input type="checkbox" name="remember" id="remember" class="w-4 h-4 text-[#A98D66] border-white/20 bg-white/10 rounded focus:ring-[#A98D66] cursor-pointer transition-all">
                        <label for="remember" class="ml-2.5 block text-xs font-bold text-slate-500 cursor-pointer select-none hover:text-white transition-colors">Ingat Sesi Saya</label>
                    </div>
                </div>

                <!-- Submit Button -->
                <button type="submit" class="w-full bg-[#3B8A7F] text-white font-black py-4 rounded-xl shadow-lg shadow-black/20 hover:bg-white hover:-translate-y-1 focus:ring-4 focus:ring-[#3B8A7F]/20 active:scale-[0.98] transition-all duration-300 text-xs uppercase tracking-widest">
                    Masuk Sekarang
                </button>
            </form>
        </div>
