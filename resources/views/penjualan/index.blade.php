...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(169,141,102,0.08)] overflow-hidden">
        <div class="p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 class="text-[#A98D66] font-black tracking-tightest uppercase text-lg">Data Transaksi Penjualan</h2>
            ...
            <div class="flex flex-col xl:flex-row justify-between items-center gap-4 mb-6 w-full">
                ...
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    <a href="{{ route('input.penjualan') }}" wire:navigate.hover class="w-full sm:w-auto bg-[#ca5b33] text-white font-black rounded-2xl shadow-xl shadow-[#ca5b33]/25 hover:bg-[#B04025] hover:-translate-y-1 active:scale-95 transition-all duration-300 px-6 py-3 text-[10px] uppercase tracking-widest flex items-center justify-center">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(169,141,102,0.08)] overflow-hidden">
        <div class="overflow-x-auto custom-scrollbar relative">
            <table id="penjualan-table" class="w-full text-left border-separate border-spacing-0">
                <thead>
                    @php
                    ...
                    <tr class="bg-[#B04025] text-white text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap">
                        ...
                        <th class="py-6 px-8 cursor-pointer group sticky left-0 bg-[#B04025] z-20 shadow-[4px_0_10px_-3px_rgba(0,0,0,0.2)] whitespace-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Tanggal_Penjualan', 'sort_dir' => $newDir, 'page' => 1]) }}'">
                            <div class="flex items-center">
                                Tanggal
                                <i class="fas {{ $currentSortBy === 'Tanggal_Penjualan' ? ($currentSortDir === 'asc' ? 'fa-sort-up text-white' : 'fa-sort-down text-white') : 'fa-sort text-white/30' }} text-[10px] ml-auto group-hover:text-white transition-colors"></i>
                            </div>
                        </th>
                        <th class="py-6 px-8 whitespace-nowrap">Informasi Produk</th>
                        <th class="py-6 px-8 whitespace-nowrap">Data Pelanggan</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($penjualans as $item)
                        <tr class="group bg-white hover:bg-[#A98D66]/10 transition-all duration-200">
                            <td class="py-5 px-8 text-sm text-slate-800 font-medium sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-[#F6F4F0] transition-colors whitespace-nowrap">{{ $item->Tanggal_Penjualan ? date('d/m/Y', strtotime($item->Tanggal_Penjualan)) : '-' }}</td>
                            ...
                            <td class="py-5 px-8 text-center whitespace-nowrap">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('data.penjualan.edit', $item->ID_Penjualan) }}" wire:navigate class="flex items-center justify-center w-8 h-8 rounded-xl bg-slate-50 text-slate-800 hover:bg-[#3B8A7F] hover:text-white shadow-sm transition-all" title="Ubah Data">
