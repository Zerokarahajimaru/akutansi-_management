...
<div class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(13,148,136,0.05)]">
 ...
 <div class="flex flex-col xl:flex-row justify-between items-center gap-4 mb-6 w-full">
    ...
    <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
        ...
        <a href="{{ route('input.penjualan') }}" wire:navigate.hover class="w-full sm:w-auto bg-[#3B8A7F] text-white font-black rounded-xl shadow-lg shadow-[#3B8A7F]/25 hover:bg-white hover:-translate-y-0.5 active:scale-95 transition-all duration-300 px-6 py-3 text-[10px] uppercase tracking-widest flex items-center justify-center">
 ...
 <th class="py-4 px-6 cursor-pointer group sticky left-0 bg-inherit z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sort_by' => 'Tanggal_Penjualan', 'sort_dir' => $newDir, 'page' => 1]) }}'">
 ...
 <td class="py-4 px-6 text-sm text-slate-800 font-medium sticky left-0 bg-white even:bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-[#A98D66]/10 whitespace-nowrap">{{ $item->Tanggal_Penjualan ? date('d/m/Y', strtotime($item->Tanggal_Penjualan)) : '-' }}</td>
