...
<div class="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgba(169,141,102,0.08)]">
 ...
 <div>
 <h2 class="text-[#A98D66] font-black tracking-tightest uppercase text-lg">Stok & Master Barang</h2>
 ...
 <td class="py-5 px-8 text-sm text-slate-800 font-bold uppercase sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] group-hover:bg-[#F6F4F0] transition-colors whitespace-nowrap">{{ $barang->ID_Barang ?? '-' }}</td>
 ...
 <td class="py-5 px-8 text-sm text-slate-800 uppercase font-medium whitespace-nowrap">{{ $barang->ID_Pemasok ?? '-' }}</td>
 <td class="py-5 px-8 text-sm text-slate-500 whitespace-nowrap">
 ...
 <td class="py-5 px-8 text-sm font-bold text-slate-400 whitespace-nowrap text-right">Rp {{ number_format($barang->Harga_Beli ?? 0, 0, ',', '.') }}</td>
 <td class="py-5 px-8 text-sm font-bold text-slate-800 whitespace-nowrap text-right">Rp {{ number_format($barang->Harga_Jual ?? 0, 0, ',', '.') }}</td>
 <td class="py-5 px-8 text-center whitespace-nowrap">
 <div class="flex justify-center space-x-2">
 <a href="{{ route('data.barang.edit', $barang->ID_Barang) }}" wire:navigate.hover class="flex items-center justify-center w-8 h-8 rounded-xl bg-slate-50 text-slate-800 hover:bg-[#3B8A7F] hover:text-white shadow-sm transition-all" title="Ubah Produk">
 <i class="fas fa-edit text-xs"></i>
 </a>
 <form action="{{ route('data.barang.destroy', $barang->ID_Barang) }}" method="POST">
 @csrf
 @method('DELETE')
 <button type="submit" class="w-8 h-8 rounded-xl bg-white text-[#B04025] border border-slate-100 hover:bg-[#B04025] hover:text-white shadow-sm transition-all" title="Hapus Produk">
