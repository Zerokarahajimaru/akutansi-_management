...
<div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(15,23,42,0.04)] border border-slate-100">
 ...
 <a href="{{ route('util.template', 'barang') }}" class="flex items-center px-4 py-2 bg-white text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-50 border border-slate-200 shadow-sm transition-all">
 ...
 <a href="{{ route('util.export', 'barang') }}" class="flex items-center px-4 py-2 bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200 shadow-sm rounded-xl text-xs font-bold transition-all">
 <i class="fas fa-file-export mr-2"></i> Ekspor Excel
 </a>
 <a href="{{ route('input.barang') }}" wire:navigate.hover class="flex items-center px-4 py-2 bg-teal-600 text-white rounded-xl text-xs font-bold hover:bg-teal-700 shadow-lg shadow-teal-500/30 transition-all hover:-translate-y-0.5">
 ...
 @csrf
 @method('DELETE')
 <button type="submit" class="w-8 h-8 rounded-lg bg-slate-50 text-rose-500 hover:bg-rose-600 hover:text-white shadow-sm transition-all" title="Hapus Produk">
